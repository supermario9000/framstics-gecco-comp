...
class FramsticksLib:
	"""Communicates directly with the Framsticks library (.dll or .so or .dylib).
	...
	GENOTYPE_INVALID_OFFSPRING_SUBSTITUTE_ORIGINAL: bool = True  # if True, when mutation or crossover is unable to perform its operation for the provided genotype(s), return the original genotype (and print a warning). If this happens extremely rarely, it may be ignored - but if not, you need to identify the reason (e.g., particular genotypes that cause the problem), fix it or change the logic of your algorithm. A more strict approach is to keep this field False - then you must always check if GENOTYPE_INVALID was returned by mutate() or crossOver(), and handle this situation properly (e.g., choose different parent(s) for mutate() or crossOver() and repeat until you get a valid offspring).
	...
	EVALUATION_SETTINGS_FILE = [  # all files MUST be compatible with the standard-eval expdef. The order they are loaded in is important!
		"eval-allcriteria.sim",  # a trade-off value for the performance sampling period ("perfperiod") that balances the conflicting requirements of vertpos and velocity
		...
		# "sample-period-2.sim", # short performance sampling period ("perfperiod") so performance (e.g. vertical position) is sampled more often
		# "sample-period-longest.sim",  # increased performance sampling period ("perfperiod") so distance and velocity are measured rectilinearly
	]
	...
	def getRandomGenotype(self, initial_genotype: str, parts_min: int, parts_max: int, neurons_min: int, neurons_max: int, iter_max: int, return_even_if_failed: bool):
		...
		:param iter_max: how many mutations can be used to generate a random genotype that fulfills target numbers of parts and neurons.
		:param return_even_if_failed: if the target numbers of parts and neurons were not achieved, return the closest genotype that was found? Set it to False first to experimentally adjust `iter_max` so that in most calls this function returns a genotype with target numbers of parts and neurons, and then you can set this parameter to True if target numbers of parts and neurons are not absolutely required.
	...
	def isValidCreature(self, genotype_list: List[str]) -> List[bool]:
		...
		pop = frams.Populations[0]  # assuming rules from population #0 (self-collision settings are population-dependent and can influence creature build success/failure)
